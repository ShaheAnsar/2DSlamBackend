...
function NodePdf(g::MvGaussian, x::Vector{T}) where T
    μ = g.μ;
    Σ = g.Σ;
    δx = x - μ;
...
function NodePdf(g::SimpleMvGaussian, x::Vector{T}) where T
    μ = g.μ;
    σ² = g.σ²;
    δx = x - μ;
    Σ = [σ² 0; 0 σ²];
...
function NodeLsqFunc(g::SimpleMvGaussian, x::Vector{T}) where T
    μ = g.μ;
    σ² = g.σ²;
    δx = x - μ;
    Σ = [σ² 0; 0 σ²];
...
function NodeLsqFunc(g::MvGaussian, x::Vector{T}) where T
    μ = g.μ;
    Σ = g.Σ;
    return transpose(δx) * inv(Σ) * δx;
end
            ...
            if computable
                ...
                if t == Position
                    ...
                    elseif b.motion_model == ConstVel
                        ...
                        NodeUpdateMean(state.dist, xnew);
                        # Calculate the joint probability
                        contrib = NodeLsqFunc(state.dist, state.value);
                        println("Contributing Factor: $contrib");
                        lsq_sum += contrib
                    elseif b.motion_model == ConstAcc
                        println("Get everything else working doofus");
            ...
            if computable
                ...
                if t == Position
                    ...
                    elseif b.motion_model == ConstVel
                        ...
                        NodeUpdateMean(state.dist, xnew);
                        # Calculate the joint probability
                        contrib = NodeLsqFunc(state.dist, m[name]);
                        println("Contributing Factor: $contrib");
                        lsq_sum += contrib
                    elseif b.motion_model == ConstAcc
                        println("Get everything else working doofus");
